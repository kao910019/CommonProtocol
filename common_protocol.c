...
void CP_GetElementValue(CP_PackagePacker *packer, uint32_t index, void *data)
{
    if (packer->configArray[index].flag & CP_FLAG_DATA_REVERSE)
        revmemcpy(data, packer->dataArray[index].address, packer->configArray[index].size);
    else
        memcpy(data, packer->dataArray[index].address, packer->configArray[index].size);
}

void CP_SetElementValue(CP_PackagePacker *packer, uint32_t index, void *data)
{
    if (packer->configArray[index].flag & CP_FLAG_DATA_REVERSE)
        revmemcpy(packer->dataArray[index].address, data, packer->configArray[index].size);
    else
        memcpy(packer->dataArray[index].address, data, packer->configArray[index].size);
}
...
bool CP_HeaderCheck(CP_PackagePacker *packer)
{
    ...
    for (int i = 0; i < packer->config->size; i++)
        printk("CP Debug: Header[%d] = 0x%02x : 0x%02x\n", i, ((uint8_t *)&packer->config->magic)[i], ((uint8_t *)&magic)[i]);
#endif
    return (memcmp((uint8_t *)&packer->config->magic, (uint8_t *)&magic, packer->config->size) == 0);
}
...
unsigned long long CP_ChecksumCalculate(CP_PackagePacker *packer)
{
    ...
    for (int arrayIndex = 0; arrayIndex < packer->elementSize; arrayIndex++)
    {
        ...
        for (int bufferIndex = 0; bufferIndex < packer->configArray[arrayIndex].size; bufferIndex++)
...
void CP_GetBindingLength(CP_PackagePacker *packer)
{
    ...
    for (int i = packer->elementIndex - 1; i > 0; i--)
    {
        ...
        {
            CP_GetElementValue(packer, i, &packer->config->size);
        }
    }
}
...
int CP_ElementPutByte(CP_PackagePacker *packer, uint8_t byte)
{
    ...
    {
        index = (packer->config->flag & CP_FLAG_DATA_REVERSE) ? (packer->config->size - 1) - packer->dataIndex : packer->dataIndex;
        if (((uint8_t *)&packer->config->magic)[index] != byte)
            return CP_FAIL;
    }
    ...
    if (packer->dataIndex == packer->config->size)
    {
#ifdef COMMON_PROTOCOL_DEBUG
        for (int i = 0; i < packer->config->size; i++)
            printk("CP Debug: %d, Put 0x%02x\n", packer->bufferIndex + 1 - packer->config->size + i, packer->buffer[packer->bufferIndex + 1 - packer->config->size + i]);
#endif
